...
ButtonsPoll(uint8_t *pui8Delta, uint8_t *pui8RawState)
{
    ...
    //Using two GPIO ports, OR together to get data bits
    ui32Data = (ROM_GPIOPinRead(BUTTONS_GPIO_BASE_A, BUTTONS_A)) |
                ROM_GPIOPinRead(BUTTONS_GPIO_BASE_F, BUTTONS_F);
...
{
    //
    // Enable the GPIO ports to which the buttons are connected.
    //
    ROM_SysCtlPeripheralEnable(BUTTONS_GPIO_PERIPH_A);
    ROM_SysCtlPeripheralEnable(BUTTONS_GPIO_PERIPH_F);

    //
    // Unlock all pins PA[2:7] to be used as GPIO pins
    // Once we have enabled (unlocked) the commit register then re-lock it
    // to prevent further changes.
    //
    HWREG(BUTTONS_GPIO_BASE_A + GPIO_O_LOCK) = GPIO_LOCK_KEY;
    HWREG(BUTTONS_GPIO_BASE_A + GPIO_O_CR) |= 0x7C;
    HWREG(BUTTONS_GPIO_BASE_A + GPIO_O_LOCK) = 0;

    //enable PF0
    HWREG(BUTTONS_GPIO_BASE_F + GPIO_O_LOCK) = GPIO_LOCK_KEY;
    HWREG(BUTTONS_GPIO_BASE_F + GPIO_O_CR) |= 0x01;
    HWREG(BUTTONS_GPIO_BASE_F + GPIO_O_LOCK) = 0;

    //
    // Set each of the button GPIO pins as an input with a pull-up.
    //
    ROM_GPIODirModeSet(BUTTONS_GPIO_BASE_A, BUTTONS_A, GPIO_DIR_MODE_IN);
    ROM_GPIODirModeSet(BUTTONS_GPIO_BASE_F, BUTTONS_F, GPIO_DIR_MODE_IN);

    MAP_GPIOPadConfigSet(BUTTONS_GPIO_BASE_A, BUTTONS_A,
                         GPIO_STRENGTH_2MA, GPIO_PIN_TYPE_STD_WPU);

    MAP_GPIOPadConfigSet(BUTTONS_GPIO_BASE_F, BUTTONS_F,
                         GPIO_STRENGTH_2MA, GPIO_PIN_TYPE_STD_WPU);
    ...
    // the GPIO bank.
    //
    g_ui8ButtonStates = ROM_GPIOPinRead(BUTTONS_GPIO_BASE_F, BUTTONS_F) |
                        ROM_GPIOPinRead(BUTTONS_GPIO_BASE_A, BUTTONS_A);
}
