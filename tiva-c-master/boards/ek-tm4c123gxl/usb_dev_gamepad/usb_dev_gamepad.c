...
ADCInit(void)
{
    ...
    //
    //ROM_GPIOPinTypeADC(GPIO_PORTA_AHB_BASE, GPIO_PIN_7 | GPIO_PIN_6 | GPIO_PIN_5
...
main(void)
{
    ...
    uint8_t ui8EncoderChanged, ui8Encoder;
    uint8_t previousEncoderValue;
    int ui8Encoder_int, previousEncoderValue_int;
    bool bUpdate;
    ...
    //
    // Configure the GPIOS for the angular encoder.
    //
    EncoderInit();

    //
    // Poll the angular encoder to initialize the most recent encoder value.
    //
    EncoderPoll(&ui8EncoderChanged, &ui8Encoder);
    previousEncoderValue = ui8Encoder;

    //
    //value to report to Z axis for the angular encoder
    //
    int valueToReport = 0;

    //
    //Loop counter to poll angular encoder every 20th iteration
    //
    int iteration = 0; 
    ...
    while(1)
    {
        ...
        if(g_iGamepadState == eStateIdle)
        {
            ...
            //ui8Buttons is an 8 bit value, place a high bit
            //in all desired locations in value to turn button on
            //Button 1 is in the LSB

            //
            // Set button 1 if P1 start pressed
            //
            if(ui8Buttons & P1_START)
            {
                sReport.ui8Buttons |= 0x01;
            }

            //
            // Set button 2 if P2 start pressed
            //
            if(ui8Buttons & P2_START)
            {
                sReport.ui8Buttons |= 0x02;
            }

            //
            // Set button 3 if up direction on joystick
            //
            if(ui8Buttons & JOYSTICK_UP)
            {
                sReport.ui8Buttons |= 0x04;
            }

            //
            // Set button 4 if down direction on joystick
            //
            if(ui8Buttons & JOYSTICK_DOWN)
            {
                sReport.ui8Buttons |= 0x08;
            }

            //
            // Set button 5 if left direction on joystick
            //
            if(ui8Buttons & JOYSTICK_LEFT)
            {
                sReport.ui8Buttons |= 0x10;
            }

            //
            // Set button 6 if left direction on joystick
            //
            if(ui8Buttons & JOYSTICK_RIGHT)
            {
                sReport.ui8Buttons |= 0x20;
            }

            //
            // Set button 7 if fire on joystick pressed
            //
            if(ui8Buttons & JOYSTICK_FIRE)
            {
                sReport.ui8Buttons |= 0x40;
            }

            //
            // See if the angular encoder updated.
            //
            EncoderPoll(&ui8EncoderChanged, &ui8Encoder);

            ui8Encoder_int = (int) ui8Encoder;
            previousEncoderValue_int = (int) previousEncoderValue;
            iteration++;

            //
            //Update value to report to Z axis on every 20th iteration
            //
            if (iteration > 20) {
              if (ui8Encoder_int < 20 && previousEncoderValue_int > 105) {
                valueToReport = 30;
              }
              else if (ui8Encoder_int > 105 && previousEncoderValue_int < 20) {
                valueToReport = -30;
              }
              else if (ui8Encoder_int > previousEncoderValue_int) {
                valueToReport = 30;
              }
              else if (ui8Encoder_int < previousEncoderValue_int) {
                valueToReport = -30;
            ...
            sReport.i8ZPos = ((uint8_t) valueToReport);

            if(ui8ButtonsChanged | ui8EncoderChanged)
            {
                bUpdate = true;
            }
